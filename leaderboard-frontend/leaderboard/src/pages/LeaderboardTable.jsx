  ...
  <td className="px-6 py-4 text-center">
    {username ? (
      <a
        ...
        className="inline-flex flex-col items-center group/link hover:bg-slate-50 px-3 py-2 rounded-lg transition-colors"
      >
        ...
        <div
          className={`text-[10px] font-bold ${
            platform === 'leetcode' ? 'text-emerald-600' : 'text-blue-600'
          }`}
        >
          Score: {score ?? 0}
        </div>
  ...
  if (users.length === 0) {
    return (
      <div className="text-center py-20 bg-white border border-dashed border-slate-300 rounded-3xl mt-8">
        ...
        <p className="text-xl font-bold text-slate-400">The leaderboard is empty. Competition is less!!</p>
      </div>
    );
  }
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-6 py-5 text-left text-xs font-bold text-slate-500 uppercase">
                Rank
              </th>
              <th className="px-6 py-5 text-left text-xs font-bold text-slate-500 uppercase">
                Coder
              </th>
              <th className="px-6 py-5 text-center text-xs font-bold text-slate-500 uppercase">
                Section
              </th>
              <th className="px-6 py-5 text-center text-xs font-bold text-slate-500 uppercase">
                LC Solved
            ...
            {users.map((user, index) => (
              <tr key={user._id || index} className="hover:bg-slate-50/80 transition-colors group">
                <td className="px-6 py-4">
                  <div
                    className={`inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br ${getRankStyle(
                      index
                    )} font-bold`}
                  >
                    {getRankIcon(index)}
                ...
                <td className="px-6 py-4">
                  <button
                    ...
                    className="font-bold text-slate-800 text-lg group-hover:text-indigo-600 flex items-center gap-1"
                  >
                    {user.name}{' '}
                  ...
                  {user.gfg ? (
                    <div>
                      <a
                        ...
                        className="text-orange-600 hover:text-orange-700 font-semibold flex items-center justify-center gap-1"
                      >
                        {user.gfg.total_problems_solved}
                        <ExternalLink className="w-3 h-3" />
                      </a>
                      <div className="text-xs text-slate-500 mt-1">
                        E:{user.gfg.easy} M:{user.gfg.medium} H:{user.gfg.hard} B:{user.gfg.basic}
                      </div>
                    </div>
                  ) : (
                    <span className="text-slate-400">-</span>
                ...
                <SolvedCell
                  ...
                  total={user.codeforces?.solved}
                  score={user.codeforces?.score}
                  url={`https://codeforces.com/profile/${user.codeforces?.username}`}
                />

                <td className="px-6 py-4 text-center font-extrabold text-lg text-slate-900">
                  {user.totalSolved ?? 0}
                </td>

                <td className="px-6 py-4 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <span className="text-xl font-black text-slate-900">{user.overallScore}</span>
                  </div>
                </td>
              </tr>
            ))}
